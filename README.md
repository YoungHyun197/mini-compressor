...
> W4A16 RTN/GPTQ/AWQ, W8A8 static/dynamic/SmoothQuant가 구현되어 있습니다.
...
│   ├── _pair_utils.py     — norm → linear 페어 탐색 유틸 (SmoothQuant/AWQ 공유)
│   ├── smoothquant.py     — SmoothQuantModifier (activation 분포 평탄화)
│   ├── gptq.py            — GPTQModifier (Hessian 기반 W4A16 weight 최적화)
│   └── awq.py             — AWQModifier (activation magnitude grid-search W4A16)
...
weight·activation은 동일한 observer 추상화를 공유합니다. `QuantizationSpec`의 `calibration_method` 필드로 방식을 고르며, observer가 정하는 것은 **clip range**이고 rounding은 어느 경우든 round-to-nearest(RTN)입니다. weight는 정적 텐서라 `initialize()`에서 observer를 1회 호출하고, activation은 calibration forward마다 호출합니다. `dynamic=True`인 activation은 런타임 scale을 쓰므로 observer가 생성되지 않습니다.
...
`python demo.py --ppl` 실행 결과 (wikitext-2 train → GPTQ calibration, wikitext-2 test → PPL 평가):
...
측정 방식: sliding window (stride=512, max\_len=2048), wikitext-2-raw-v1 test split.
...
| `test_awq.py` | AWQModifier 등가 변환, LayerNorm bias, interface 계약, Compressor chain |
...
- [x] **AWQ** (`AWQModifier`) — activation magnitude grid-search per-channel scaling, `from_recipe("w4a16_awq")`
