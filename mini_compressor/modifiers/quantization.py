# QuantizationModifier — RTN 기반 weight/activation 양자화 modifier
...
class QuantizationModifier(BaseModifier):
    """nn.Linear를 FakeQuantLinear로 교체하고 RTN 기반 scale을 계산한다.

    Lifecycle:
        initialize(model): nn.Linear → FakeQuantLinear 교체 + weight scale RTN (compute_scales=True 기본).
